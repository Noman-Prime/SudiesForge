...
export const createTopic = async (req) => {
    try {
        await connect()
        const { chapter, topicNumber, topicName, sections } = await req.json()
        const Topic = await topic.create({ chapter, topicNumber, topicName, sections })
...
export const updateTopic = async (req, id) => {
    try {
        await connect()
        const { chapter, topicNumber, topicName, sections } = await req.json()
        const Topic = await topic.findByIdAndUpdate(id, { chapter, topicNumber, topicName, sections }, { new: true, runValidators: true })
        if (!Topic) {
            return NextResponse.json({
                success: false,
...
export const createOrUpdateImage = async (req, id) => {
    try {
        ...
        const oldImage = Topic.image?.public_id
        const imageData = await uploadFile(file, `studies/Topic/${id}/Image`, "image")
        if (oldImage) {
            await deleteFile(oldImage, "image")
        }
        Topic.image = {
